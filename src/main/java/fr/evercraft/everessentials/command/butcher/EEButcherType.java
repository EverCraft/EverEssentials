...
public class EEButcherType extends ESubCommand<EverEssentials> {
	...
	public Optional<EntityType> getEntityType(String name) {
		...
		return Streams.concat(UtilsEntityType.ANIMALS.stream(), UtilsEntityType.MONSTERS.stream())
			.filter(entity -> entity.getId().equalsIgnoreCase(type) || entity.getName().equalsIgnoreCase(type))
			.findAny();
	}
}
