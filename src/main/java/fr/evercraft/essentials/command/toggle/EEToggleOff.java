...
public class EEToggleOff extends ESubCommand<EverEssentials> {
	...
	public Text description(final CommandSource source) {
		return EEMessages.TOGGLE_OFF_DESCRIPTION.getText();
	}
	...
	public CompletableFuture<Boolean> execute(final CommandSource source, final List<String> args) throws CommandException {
		if (args.size() == 0) {
			if (source instanceof EPlayer) {
				return this.commandToggleOff((EPlayer) source);
			} else {
			...
			if (source.hasPermission(EEPermissions.TOGGLE_OTHERS.get())){
				...
				// Le joueur existe
				if (user.isPresent()){
					return this.commandToggleOffOthers(source, user.get());
	...
	private CompletableFuture<Boolean> commandToggleOff(final EPlayer player) {
		// Toggle désactivé
		if (!player.isToggle()) {
			EEMessages.TOGGLE_OFF_PLAYER_ERROR.sendTo(player);
			return CompletableFuture.completedFuture(false);
		}
	...
	private CompletableFuture<Boolean> commandToggleOffOthers(final CommandSource staff, final EUser user) throws CommandException {
		...
		// Toggle désactivé
		if (!user.isToggle()) {
			EEMessages.TOGGLE_OFF_OTHERS_ERROR.sender()
		...
		if (!user.setToggle(false)) {
			EEMessages.TOGGLE_OFF_OTHERS_CANCEL.sender()
				.replace("<player>", user.getName())
				.sendTo(staff);
			return CompletableFuture.completedFuture(false);
		}
		
		EEMessages.TOGGLE_OFF_OTHERS_STAFF.sender()
			.replace("<player>", user.getName())
			.sendTo(staff);
		
		if(user instanceof EPlayer) {
			EEMessages.TOGGLE_OFF_OTHERS_PLAYER.sender()
